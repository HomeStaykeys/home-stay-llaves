/* ============================================================
   Home Stay · Capa de datos — Firestore (nube) + Local seguro
   ------------------------------------------------------------
...
(function () {
  ...
  const COL = window.FIREBASE_COLLECTION || "registros_llaves";
  const canFirebase = !!(cfg.apiKey && cfg.projectId && window.firebase);

  /* ---- claves locales (respaldo / espejo / export) ---- */
  const KEY = "homestay_llaves_v1";
  const BAK = "homestay_llaves_backups_v1";
  const SEEDFLAG = "homestay_seeded_v1";
  const LEGACY_KEYS = [];
  const MAX_BAK = 40;
  ...
  const KeyDB = {
    ...
    exportJSON() {
      return JSON.stringify({ obra: "Home Stay", modo: mode, exportadoEl: new Date().toISOString(), registros: this.all() }, null, 2);
    },
