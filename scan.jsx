...
function ScanApp() {
  ...
  const [done, setDone] = useState(null);   // {tipo, rows}
  ...
  return (
    <div className="scan-body">
      <div className="scan-hero">
        <div className="scan-hero-tag">Obra · Edificio Home Stay</div>
...
function RetiroForm({ onCancel, onDone }) {
  ...
  const personaOk = f.persona.trim() && f.documento.trim() && f.empresa.trim() && f.celular.trim();
...
function DoneScreen({ data, onReset }) {
  ...
  return (
    <div className="scan-body done">
      ...
      <h1>{retiro ? (multi ? "Llaves retiradas" : "Llave retirada") : (multi ? "Llaves entregadas" : "Llave entregada")}</h1>
      <p>{retiro
        ? (multi ? `${rows.length} llaves quedaron asentadas en el site con estado Abierto.` : "El registro quedó asentado en el site con estado Abierto.")
        : (multi ? `${rows.length} llaves fueron devueltas y cerradas en el registro.` : `Registro cerrado. El departamento estuvo ${diasTexto(r.dias)} en poder de la empresa.`)}</p>
      <div className="done-card">
        ...
        {retiro && r.celular ? <div className="dc-row"><span>Celular</span><b className="mono">{r.celular}</b></div> : null}
        <div className="dc-row"><span>{retiro ? "Retiro" : "Entrega"}</span><b>{fmtFecha(retiro ? r.retiroAt : r.entregaAt)}</b></div>
        <div className="dc-row"><span>Estado</span><StatusBadge estado={retiro ? "abierto" : "cerrado"} /></div>
      </div>
